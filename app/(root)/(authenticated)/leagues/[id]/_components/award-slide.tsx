...
const AwardSlide = forwardRef<HTMLDivElement, AwardSlideProps>(
  ({ award, shareButton }, ref) => {
    return (
      <div
        ref={ref}
        className={`flex-shrink-0 w-full h-full bg-gradient-to-br ${award.gradient} flex flex-col items-center justify-center text-white px-8 relative`}
      >
        {award.image ? (
          <img
        ...
        <p className="text-xs sm:text-sm text-white/50 italic mb-8 px-8 text-center">
          {award.description}
        </p>
